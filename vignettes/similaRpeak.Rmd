...
Mathematicaly, a metric is considered as a function that quantifies the 
similarity between two objects.The function must return zero when the two 
objects are similar (identity of indiscernibles) and a non-negative value when 
are dissimilar. 

The metrics present in the **similaRpeak** package do not strictly respect this
standard but can all be translated to respect it.

By using the absolute value of the **DIFF_POS_MAX** metric, the definition of 
a metric is formally respected. However, the respective position of the 
...
**RATIO_NORMALIZED_INTERSECT** metrics, the definition of a metric is formally 
respected.


## Metrics calculation - General approach
...
par(new=TRUE)
plot(H3K4me1_profile, type="l", col="darkgreen", xlab="Position", ylab="Reads", 
            ylim=c(0, 700))
...
are quite low. Both ratios values can be explained by the large difference in 
...
This hypothesis can be validated by a graph of the normalized profiles :
...
plot(H3K4me1_profile*length(H3K4me1_profile)/sum(H3K4me1_profile, na.rm=T), 
     type="l", col="darkgreen", xlab="Position", ylab="Normalized Reads", 
            ylim=c(0, 3.5))
legend("topleft", c("H3K27ac","H3K4me1"), cex=1.2, 
            col=c("blue","darkgreen"), lty=1)
```
